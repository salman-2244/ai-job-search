...
run-scoped layout, used by supervised runs so two runs on one day never overwrite
each other's documents). Take it verbatim — **never** reinterpret it as a
`<slug>` prefix of a different location.
